...
def analyze(name, sym, tf_label, tf_bar):
    ...
    if funding <= -0.02:
        ls += 2
        reasons_l.append("資金費率極低"+str(funding)+"%")
    elif funding < 0:
        ls += 1
    if funding >= 0.05:
        ss += 2
        reasons_s.append("資金費率極高"+str(funding)+"%")
    elif funding > 0:
        ss += 1
    if lsr <= 0.8:
        ls += 2
        reasons_l.append("散戶極度做空 LSR:"+str(lsr))
    elif lsr <= 0.95:
        ls += 1
    if lsr >= 1.3:
        ss += 2
        reasons_s.append("散戶極度做多 LSR:"+str(lsr))
    elif lsr >= 1.1:
        ss += 1
    if rsi_val <= 30:
        ls += 2
        reasons_l.append("RSI超賣:"+str(rsi_val))
    elif rsi_val <= 45:
        ls += 1
    if rsi_val >= 70:
        ss += 2
        reasons_s.append("RSI超買:"+str(rsi_val))
    elif rsi_val >= 55:
        ss += 1
    if price > ema20 > ema50:
        ls += 2
        reasons_l.append("均線多頭排列")
    elif price < ema20 < ema50:
        ss += 2
        reasons_s.append("均線空頭排列")
    if price <= bb_l:
        ls += 1
        reasons_l.append("觸碰布林下軌")
    if price >= bb_u:
        ss += 1
        reasons_s.append("觸碰布林上軌")
    if vol_ratio >= 1.5:
        if price > closes[-2]:
            ls += 1
        else:
    ...
    if htf_bias == "LONG":
        ls += 3
        reasons_l.append("高週期多頭共鳴")
    elif htf_bias == "SHORT":
        ss += 3
        reasons_s.append("高週期空頭共鳴")
